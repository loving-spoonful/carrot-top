...
if (Meteor.isServer) {
    var myPostLogout = function (userId, info) {
        ...
        var selector = {};
        selector[key] = value;
        console.log(Meteor.users.findOne(selector));
        console.log(Meteor.users.findOne({_id: " + userId + "}));
        console.log(Meteor.users.findOne({_id: userId}));
        //mcpmcp send an smtp email to the administrator that a new user was requested.
        Meteor.call('sendEmail',
            CTOP_REDIRECT_EMAIL_FOR_TESTING,
            CTOP_REDIRECT_EMAIL_FOR_TESTING,
            'New User Registered',
            'Check and see if you should approve ' + Meteor.users.findOne(selector).emails[0].address);
